...
def deleteExtraFiles(downloadFolder, file_path):
    # Eliminar todos los archivos .jpg y .webm de la carpeta de descarga
    for file_name in os.listdir(downloadFolder):
        file_path = os.path.join(downloadFolder, file_name)
        if file_path.startswith(file_path) and not file_path.endswith('.mp3'):
            os.remove(file_path)
            print(f"Eliminado: {file_path}")
...
if __name__ == "__main__":
    ...
    for video in videoList:
        ...
        deleteExtraFiles(downloadFolder, videoInfoDic.get('id', ''))
